...
class RekapPerhitunganLemburExport implements FromQuery, WithMapping, ShouldAutoSize, WithEvents, WithCustomStartCell, WithTitle
{
    ...
    {
        $dateRange = 'tanggal_berjalan between concat(substr("' . $this->daterange1[0] . '", 7, 4),"-",substr("' . $this->daterange1[0] . '", 1, 2),"-",substr("' . $this->daterange1[0] . '", 4, 2)) and concat(substr("' . $this->daterange1[1] . '", 7, 4),"-",substr("' . $this->daterange1[1] . '", 1, 2),"-",substr("' . $this->daterange1[1] . '", 4, 2))';

        $q =  RekapPerhitunganLembur::query()
                 ->whereRaw('
                    ' . $dateRange . '
                ')
                ->orderBy('tanggal_berjalan','asc')
                ->limit(1);
